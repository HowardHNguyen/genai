...
# Sidebar for user input with low-risk defaults
st.sidebar.header("Enter Your Parameters")
sex = st.sidebar.selectbox("SEX (0 = Female, 1 = Male)", [0, 1], index=0)
age = st.sidebar.slider("AGE", 32.0, 81.0, 32.0)  # Low-risk default
educ = st.sidebar.slider("Education Level (educ)", 1.0, 4.0, 2.0)
cursmoke = st.sidebar.selectbox("Current Smoker (0 = No, 1 = Yes)", [0, 1], index=0)  # Low-risk default
cigpday = st.sidebar.slider("Cigarettes per Day", 0.0, 90.0, 0.0)
totchol = st.sidebar.slider("Total Cholesterol", 107.0, 696.0, 150.0)  # Low-risk
sysbp = st.sidebar.slider("Systolic BP", 83.5, 295.0, 90.0)  # Low-risk
diabp = st.sidebar.slider("Diastolic BP", 30.0, 159.0, 60.0)  # Low-risk
bmi = st.sidebar.slider("BMI", 15.0, 59.0, 20.0)  # Healthy
heartrte = st.sidebar.slider("Heart Rate", 40.0, 120.0, 60.0)  # Low-risk
